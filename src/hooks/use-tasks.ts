...
const toast = {
  ...
  init: (...args: unknown[]): void => {
    console.warn('toast.init not implemented', args);
  },

  getValue: <T = unknown,>(key: string): T | undefined => {
    console.warn('toast.getValue not implemented', key);
    return undefined;
  },

  setValue: <T = unknown,>(key: string, value: T): void => {
    console.warn('toast.setValue not implemented', key, value);
  }
} as const;const toast = { _stub: true, init: (...args: unknown[]): void => {console.warn('toast.init not implemented', args);}, getValue: <T = unknown,>(key: string): T | undefined => {console.warn('toast.getValue not implemented', key);return undefined;}, setValue: <T = unknown,>(key: string, value: T): void => {console.warn('toast.setValue not implemented', key, value);} } as const;export interface TaskFilters {search: string;type: TaskType | 'all';mode: TaskMode | 'all';minCredits: number;maxCredits: number;}const MOCK_TASKS: Task[] = [{ id: '1', creator_id: 'mock-user-1', type: 'offer', title: 'Guitar Lessons for Beginners', description: 'I can teach you the basics of acoustic guitar. We will cover chords, strumming patterns, and simple songs. I have 5 years of teaching experience.', estimated_credits: 1, mode: 'online', status: 'open',
  visibility: 'public',
  max_participants: 1,
  travel_allowance: 0,
